...
- **Provider 抽象**：`ENRICH_PROVIDERS: dict[str, EnrichProvider]`，每个 provider 实现 `query(ips) -> dict[ip, (asn,country,org)]`。当前实现至少两个：
  - `cymru`：DNS 查 `AS{ip}.origin.asn.cymru.com` TXT，纯 DNS、无账号。**（实现优化项 C，不影响架构）**：大批量时改用 cymru port-43 批量 whois 接口（一次 TCP 连接喂多个 IP，吞吐高一个量级），DNS TXT 仅作单 IP 兜底。默认实现选型写码时定。
  - `ipapi`：POST `http://ip-api.com/batch`（≤100/批），走 `*_PROXY`（GFW 兼容），返回 ASN+country+org+asname。
- **不预设主/备**：默认 provider 顺序、用哪个，等最后统一实测后定（当前默认尝试顺序 `cymru → ipapi`，但**概念上去掉主备**，是「可用 provider 列表 + 回退链」，任一成功即采用，全失败才 `ASN=-`）。写码时实测国内/WSL/VPS 各环境解析一致性后固化默认值。
- **（字段语义注记 D，非架构问题）**：cymru 的 `country` 是「该 ASN 的**注册国**」，并非 IP **地理国**；ip-api 才是真地理国。报告 Country 列若来源为 cymru，实为 AS 注册国。建议在缓存/报告标注 country 来源，或统一以 ip-api 地理国为主、cymru 仅补 ASN。文档点明避免误读。
- **国家字段默认开**：`cymru` 一次 DNS 查询即可带出 ASN+国家（非两次请求），成本近似零；④ 本就补身份，国家是身份一部分 → 默认 `--enrich-country` 开，`--no-enrich-country` 关。

### ④.4 缓存策略（永久缓存 + TTL + 失败回退）
- 每 IP 缓存：`cache/asn_{ip}.txt` = `asn|country|org|source|cached_at`，**永久保留**。
- TTL 24h：超过 24h 优先用「在线 provider 刷新」；**刷新失败 → 直接用旧缓存**（不降级为 `ASN=-`）。离线能力强。
- 首次无缓存：在线查，失败则 `ASN=-`/`Country=-`（仅首次、且遵守铁律不丢 IP）。
- `cached_at` 记录时间戳，报告/调试可见数据新鲜度。

### ④.5 输出 + source 溯源字段
- `block_NNN.enrich.txt` 每行：`IP:Port ASN COUNTRY ORG IS_CF_OFFICIAL SOURCE CACHED_AT`
  - `SOURCE` ∈ {`cymru`, `ipapi`, `cache`}：标注该行数据来自哪个 provider / 是否旧缓存。以后某 ASN 查错，一眼溯源，不用猜。
...
### ④.6 并发与限速
- `cymru`：支持批量查（一次多 IP），自身走 DNS 不受 CPU 并发限；礼貌并发（写码实测 DNS 限速，默认保守值）。
- `ipapi`：**单独保守并发上限**（默认 32，避免代理通道打满连累 ① 抓取）；`/batch` ≤100/批，按免费额度令牌桶限速（45 req/min）。
- 断点：④ 按 block 落盘；单 IP 全源失败 → `ASN=-` 保留不丢（铁律 ⑨）。
